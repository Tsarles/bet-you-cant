  ...
  return (
    <div className="landing" onClick={() => navigate('/home')}>
      ...
      {DOODLE_CARDS.map((card, i) => (
        <div key={i} className={`fl-card ${card.cls}`} title={card.label||undefined}>
      ...
      <div className="landing__footer">
        <span>
          <i className="bx bx-copyright" style={{verticalAlign:'middle'}} /> 2026 Bet You Can't · Built by&nbsp;
        ...
        <button className="landing__footer-link" onClick={e=>{e.stopPropagation();setShowSuggest(true)}}>
          Suggest a Game
        </button>
        <span className="landing__footer-sep">·</span>
        <a href="#privacy" className="landing__footer-link"
          onClick={e=>{e.stopPropagation();e.preventDefault();document.getElementById('privacy-modal').showModal()}}>
          Privacy &amp; Terms
        </a>
      </div>

      {/* Privacy modal */}
      <dialog id="privacy-modal" className="privacy-modal"
        onClick={e=>{if(e.target===e.currentTarget)e.currentTarget.close()}}>
        <div className="privacy-modal__inner">
          <button className="privacy-modal__close btn btn-ghost"
          ...
          <div className="privacy-modal__body">
            ...
            <section>
              <h3>Data &amp; Privacy</h3>
              <p>Game sessions are stored in <strong>Firebase Firestore</strong> temporarily to enable real-time multiplayer. No personal data is collected, sold, or shared with third parties. Sessions expire automatically.</p>
            </section>
            <section>
              <h3>Fair Play</h3>
